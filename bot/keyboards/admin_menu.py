...
def get_users_list_menu(users: list, page: int = 0, lang: str = "ru") -> InlineKeyboardMarkup:
    """Меню со списком пользователей (с пагинацией)"""
    keyboard = []
    ...
    for user in page_users:
        # Проверяем тип: если объект, используем .is_banned, если словарь - .get()
        if hasattr(user, 'is_banned'):
            status = "🔴" if user.is_banned else "🟢"
            name = user.first_name[:20] if user.first_name else str(user.user_id)
            user_id = user.user_id
        else:
            status = "🔴" if user.get('is_banned') else "🟢"
            name = user.get('first_name', str(user.get('user_id')))[:20]
...
def get_users_list_menu(users: list, page: int = 0, lang: str = "ru") -> InlineKeyboardMarkup:
    ...
    for user in page_users:
        ...
        name = user.first_name[:20] if user.first_name else str(user.user_id)
        keyboard.append([InlineKeyboardButton(
            text=f"{status} {name} ({user.user_id})",
...
def get_user_actions_menu(user_id: int, is_banned: bool, lang: str) -> InlineKeyboardMarkup:
    """Меню действий с конкретным пользователем"""
